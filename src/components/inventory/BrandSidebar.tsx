...
export const BrandSidebar: React.FC<BrandSidebarProps> = ({ brands, selectedBrandId, onSelectBrand, onAddBrand, onBrandDeleted, totalProducts }) => {
  const handleDeleteBrand = async (brandId: string, brandName: string, productCount: number) => {
    const confirmMessage =
      productCount > 0
        ? `Are you sure you want to delete "${brandName}"?\n\n${productCount} product(s) will have their brand removed (set to no brand).`
        : `Are you sure you want to delete "${brandName}"?`;

    if (!confirm(confirmMessage)) return;

    try {
      // Delete the brand (products will have brand_id set to NULL due to ON DELETE SET NULL)
      await dbService.execute("DELETE FROM brands WHERE id = ?", [brandId]);

      // Enqueue sync
      await dbService.enqueueSync("DELETE_BRAND", brandId, {
        id: brandId,
    ...
    <div className='h-full flex flex-col bg-white dark:bg-dark-surface border-r border-slate-200 dark:border-dark-border'>
      ...
      <div className='p-4 border-b border-slate-200 dark:border-dark-border'>
        <div className='flex items-center justify-between mb-3'>
          <h2 className='text-lg font-bold text-slate-900 dark:text-white'>Brands</h2>
          <button onClick={onAddBrand} className='p-2 rounded-lg bg-primary text-white hover:bg-primary/90 transition-colors' title='Add new brand'>
            <Plus size={16} />
          </button>
        </div>
      </div>
      ...
      <div className='flex-1 overflow-y-auto p-2'>
        ...
        {brands.map((brand) => (
          ...
          >
            <button onClick={() => onSelectBrand(brand.id)} className='flex-1 flex items-center justify-between text-left'>
